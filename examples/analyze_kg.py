...
def create_clean_network_viz(
    G,
    title="Network",
    node_attr=None,
    figsize=(18, 14),
    min_label_degree=None,
    max_labels=15,
):
    ...
    if G.is_directed():
        ...
        nx.draw_networkx_edges(
            G, pos, alpha=edge_alphas, width=edge_widths, edge_color="#666666"
        )
    ...
    important_nodes = sorted(
        [(node, deg) for node, deg in degrees.items() if deg >= min_label_degree],
        key=lambda x: x[1],
        reverse=True,
    )[:max_labels]
...
def create_concept_cooccurrence_network_original(
    facts_df, links_df, concepts_df, min_cooccurrence=2
):
    ...
    strong_cooccurrences = {
        pair: count
        for pair, count in concept_cooccurrence.items()
    ...
    for (c1, c2), freq in sorted(
        concept_cooccurrence.items(), key=lambda x: x[1], reverse=True
    )[:10]:
    ...
    nx.draw_networkx_edges(
        G_cooccur, pos, width=edge_widths, alpha=0.6, edge_color="gray"
    )
...
def create_fact_network_original(facts_df, links_df):
    ...
    for _, link in fact_to_fact_links.iterrows():
        source_short = (
            link["source"][:50] + "..." if len(link["source"]) > 50 else link["source"]
        )
        target_short = (
            link["target"][:50] + "..." if len(link["target"]) > 50 else link["target"]
        )
        G_facts.add_edge(source_short, target_short)

    print(
        f"Fact network has {len(G_facts.nodes())} nodes and {len(G_facts.edges())} edges"
    )
    ...
    for fact, degree in sorted(in_degrees.items(), key=lambda x: x[1], reverse=True)[
        :5
    ]:
        print(f"  {degree} refs: {fact}")

    print("\nFacts that reference many others (high out-degree):")
    for fact, degree in sorted(out_degrees.items(), key=lambda x: x[1], reverse=True)[
        :5
    ]:
        print(f"  {degree} refs: {fact}")
    ...
    nx.draw_networkx_edges(
        G_facts, pos, alpha=0.6, arrows=True, arrowsize=15, edge_color="gray", width=1
    )
    ...
    high_degree_nodes = [
        node for node in G_facts.nodes() if (in_degrees[node] + out_degrees[node]) > 1
    ]
    ...
    plt.title(
        "Original Fact-to-Fact Reference Network\n(node size = how often referenced)",
        fontsize=14,
    )
    plt.axis("off")
    plt.tight_layout()
...
def create_centrality_influence_analysis_original(concepts_df, facts_df, links_df):
    ...
    plt.title(
        "Original Concept Degree Centrality Analysis\n(Direct Connections in Network)"
    )
...
def create_concept_cooccurrence_network(facts_df, links_df, min_cooccurrence=2):
    ...
    sorted_cooccur = sorted(
        strong_cooccurrences.items(), key=lambda x: x[1], reverse=True
    )[:10]
