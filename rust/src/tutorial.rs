use crate::cron::{CronJobKind, CronRegistry, parse_duration};
use crate::tui::{
    RawModeGuard, clear_all, footer_line, header_line, hide_cursor, term_size, wline,
};
...
pub fn run_tutorial_tui(
    ...
    loop {
        ...
        match step {
            ...
            Step::Model => render_model(
                &mut out, cols, body_start, body_end, &model, &provider, editing, &edit_buf, cursor,
            ),
            Step::Provider => render_provider(
                &mut out, cols, body_start, body_end, &provider, editing, &edit_buf, cursor,
            ),
            Step::ApiKeys => render_api_keys(
                &mut out,
                cols,
                body_start,
                body_end,
            ...
            Step::Heartbeats => render_heartbeats(
                &mut out,
                cols,
                body_start,
                body_end,
        ...
        let foot = match step {
            ...
            _ => {
                if editing {
                    ...
                    " [j/k] Nav  [e] Edit  [Enter] Next  [b] Back  [q] Quit "
                }
            }
        };
        ...
        if let Ok(Event::Key(key)) = event::read() {
            ...
            if editing {
                match c {
                            ...
                            Step::ApiKeys => {
                                if !edit_label.is_empty() {
                                    if edit_buf.trim().is_empty() {
                                        unsafe {
                                            std::env::remove_var(&edit_label);
                                        }
                                    } else {
                                        unsafe {
                                            std::env::set_var(&edit_label, edit_buf.trim());
                                        }
                                    }
                                }
                            }
                    ...
                    KeyCode::Backspace => {
                        if cursor > 0 {
                            cursor -= 1;
                            edit_buf.remove(cursor);
                        }
                    }
                    KeyCode::Delete => {
                        if cursor < edit_buf.len() {
                            edit_buf.remove(cursor);
                        }
                    }
                    KeyCode::Left => {
                    ...
                    KeyCode::Char(ch) => {
                        if key.modifiers == KeyModifiers::CONTROL && ch == 'c' {
                            break;
                        }
                        edit_buf.insert(cursor, ch);
                        cursor += 1;
                    }
                    _ => {}
                }
            ...
            match c {
                ...
                KeyCode::Char('e') | KeyCode::Enter => match step {
                    Step::Model => {
                        editing = true;
                        edit_buf = model.clone();
                        cursor = edit_buf.len();
                    }
                    Step::Provider => {
                        editing = true;
                        edit_buf = provider.clone();
                        cursor = edit_buf.len();
                    }
                    Step::ApiKeys => {
                        editing = true;
                ...
                KeyCode::Char('j') | KeyCode::Down => match step {
                    Step::ApiKeys => {
                        if hb_sel < 3 {
                            hb_sel += 1;
                        }
                    ...
                    Step::Heartbeats => {
                        ...
                        if hb_field > 2 {
                            hb_field = 0;
                            hb_sel = (hb_sel + 1).min(heartbeats.len().saturating_sub(1).max(0));
                        }
                    }
                    Step::Skills => {
                        ...
                        keys.sort();
                        if let Some(idx) =
                            keys.iter().position(|k| *skills.get(k).unwrap_or(&false))
                        {
                        }
                    }
                    _ => {}
                },
                KeyCode::Char('k') | KeyCode::Up => match step {
                    Step::ApiKeys => {
                        if hb_sel > 0 {
                            hb_sel -= 1;
                        }
...
fn render_welcome(out: &mut io::Stdout, cols: usize, start: usize, end: usize) {
    ...
    for (i, line) in lines.iter().enumerate() {
        ...
        wline(
            out,
            start + i,
            &format!("  {}", line.chars().take(cols - 4).collect::<String>()),
        );
    }
}
...
fn render_model(
    ...
    wline(
        out,
        start + 1,
        &format!("  Active model: {} ({})", model, provider),
    );
    wline(
        out,
        start + 3,
        "  Examples: qwen3.5:9b, gpt-4o, claude-3-5-sonnet-20241022",
    );
    if editing {
...
fn render_provider(
    ...
    wline(
        out,
        start + 3,
        "  Examples: ollama, openai, anthropic, gemini, deepseek, lmstudio",
    );
    if editing {
...
fn render_heartbeats(
    ...
    wline(
        out,
        start + 1,
        "  Recurring tasks run in the background and report to the shell.",
    );
    if heartbeats.is_empty() {
        wline(
            out,
            start + 3,
            "  \x1b[90mNo heartbeats configured. Press [a] to add one.\x1b[0m",
        );
    } else {
        for (i, (npc, interval, task)) in heartbeats.iter().enumerate() {
            ...
            wline(
                ...
                &format!(
                    ...
                    interval,
                    f2,
                    task.chars().take(cols - 45).collect::<String>()
                ),
            );
        }
    }
    if editing {
        ...
        wline(
            ...
            "  [a] Add  [d] Delete  [e] Edit  [j/k] Navigate",
        );
    }
}
...
fn render_skills(
    ...
    wline(
        out,
        start + 1,
        "  Toggle skills to install as jinxes in your team.",
    );
    let mut keys: Vec<String> = skills.keys().cloned().collect();
    keys.sort();
...
fn render_finish(
    ...
    wline(
        out,
        start + 3,
        &format!("  Heartbeats: {}", heartbeats.len()),
    );
    for (i, (npc, interval, task)) in heartbeats.iter().enumerate() {
        wline(
            out,
            start + 4 + i,
            &format!(
                "    - @{npc} every {interval}: {task}",
                task = task.chars().take(cols - 35).collect::<String>()
            ),
        );
    }
    ...
    wline(
        out,
        start + 7 + heartbeats.len(),
        "  [Enter] Save everything to ~/.npcsh and finish",
    );
}
...
fn finish_tutorial(
    ...
    if let Ok(content) = std::fs::read_to_string(&rc_path) {
        for line in content.lines() {
            ...
            {
                if k.trim() == "NPCSH_CHAT_MODEL" {
                    lines.push(format!("export NPCSH_CHAT_MODEL={}", model));
                    found_model = true;
                    continue;
                }
                if k.trim() == "NPCSH_CHAT_PROVIDER" {
                    lines.push(format!("export NPCSH_CHAT_PROVIDER={}", provider));
                    found_provider = true;
                    continue;
                }
            }
    ...
    let skills_dir =
        PathBuf::from(shellexpand::tilde("~/.npcsh/npc_team/jinxes/skills").to_string());
    ...
    if !heartbeats.is_empty() {
        println!(
            "\x1b[36m{} heartbeat(s) registered.\x1b[0m",
            heartbeats.len()
        );
    }
}
