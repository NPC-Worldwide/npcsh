...
if not sys.stdin.isatty():
    ...
    if rc != 0:
        ...
        try:
            ...
            init_phase = 'menu'  # menu, remote_input
            remote_buf = ''
            init_msg = ''
            ...
            def render_init():
                ...
                try:
                    ...
                    width, height = s.columns, s.lines
                except:
                    pass
                out = []
                hdr = ' GIT: ' + os.path.basename(repo_path) + '  (no repo) '
                out.append('\033[1;1H\033[K\033[7;1m' + hdr.ljust(width) + '\033[0m')
                out.append('\033[3;1H\033[K\033[90m' + ('-' * width) + '\033[0m')

                if init_phase == 'menu':
                    out.append('\033[5;3H\033[1mNo git repository found in:\033[0m')
                    out.append('\033[6;5H\033[90m' + repo_path + '\033[0m')
                    out.append('\033[8;3H\033[33m[i]\033[0m  Initialize a new repository')
                    out.append('\033[9;3H\033[33m[q]\033[0m  Quit')
                elif init_phase == 'remote_input':
                    out.append('\033[5;3H\033[1;32mRepository initialized.\033[0m')
                    out.append('\033[7;3H\033[1mRemote URL\033[0m \033[90m(leave empty to skip):\033[0m')
                    out.append('\033[8;5H' + remote_buf + '\033[7m \033[0m')
                    out.append('\033[10;3H\033[90mEnter to confirm, Esc to skip\033[0m')

                if init_msg:
                    out.append('\033[' + str(height - 2) + ';3H\033[32m' + init_msg[:width - 4] + '\033[0m')

                # Clear remaining lines
                for row in range(12, height - 2):
                    out.append('\033[' + str(row) + ';1H\033[K')
                out.append('\033[' + str(height) + ';1H\033[K\033[7m [i] Init  [q] Quit \033[0m')
            ...
            while init_running:
                ...
                if init_phase == 'menu':
                    if c in ('q', '\x03'):
                        ...
                        irc, _, ierr = run_git('init')
                        if irc == 0:
                            init_phase = 'remote_input'
                            remote_buf = ''
                            init_msg = 'Initialized empty git repository'
                        else:
                            init_msg = 'Init failed: ' + ierr[:60]
                        render_init()
                    ...
                    if c == '\x1b':
                        ...
                        if remote_buf.strip():
                            rrc, _, rerr = run_git('remote', 'add', 'origin', remote_buf.strip())
                            if rrc == 0:
                                init_msg = 'Remote added: ' + remote_buf.strip()
                            else:
                                init_msg = 'Remote failed: ' + rerr[:60]
                        init_done = True
                        init_running = False
                    elif c == '\x7f' or c == '\b':
                        remote_buf = remote_buf[:-1]
                        render_init()
                    elif c.isprintable():
                        remote_buf += c
                        render_init()
    ...
    if rc != 0:
        ...
        def load_commit_files(commit_hash):
            """Load files changed in a commit with status."""
            _, out, _ = run_git('diff-tree', '--no-commit-id', '-r', '--name-status', commit_hash)
            files = []
            for line in out.splitlines():
                parts = line.split('\t')
                if len(parts) >= 2:
                    files.append({'status': parts[0], 'path': parts[-1], 'selected': True})
            return files
        ...
        class St:
            ...
            stashes = []
            # Modes
            mode = 'list'  # list, diff, commit_msg, commit_detail, cherry_pick
            diff_lines = []
            diff_scroll = 0
            prev_mode = 'list'  # track where to go back to from diff
            # Cherry-pick file selection
            cherry_hash = ''
            cherry_subject = ''
            cherry_files = []  # [{status, path, selected}, ...]
            cherry_sel = 0
            cherry_scroll = 0
            msg = ''
            msg_color = '33'
        ...
        def refresh_status():
            ...
            # Actions section
            st.all_files.append({'type': 'header', 'label': 'Actions'})
            st.all_files.append({'type': 'action', 'action': 'push', 'label': 'Push to remote'})
            st.all_files.append({'type': 'action', 'action': 'pull', 'label': 'Pull from remote'})
            st.all_files.append({'type': 'action', 'action': 'fetch', 'label': 'Fetch all remotes'})
            st.all_files.append({'type': 'action', 'action': 'diff_main', 'label': 'Diff vs main'})
            # Clamp selection
            if st.all_files:
        ...
        def render():
            ...
            elif st.mode == 'cherry_pick':
                render_cherry_pick(out, width, height)
            elif st.tab == 0:
            ...
            if st.mode == 'diff':
                ...
                foot = ' [b] Back  [j/k] Scroll  [d] File picker  [q] Quit '
            elif st.mode == 'cherry_pick':
                sel_count = sum(1 for f in st.cherry_files if f['selected'])
                foot = ' [Space] Toggle  [a] All  [n] None  [Enter] Apply ' + str(sel_count) + '/' + str(len(st.cherry_files)) + '  [d] Diff  [b] Back '
            elif st.tab == 0:
                foot = ' [j/k] Nav  [Enter] Open  [s] Stage  [a] StageAll  [u] UnstageAll  [c] Commit  [Tab] Switch  [q] Quit '
            elif st.tab == 1:
                foot = ' [j/k] Nav  [Enter] Detail  [p] Pick files  [Tab] Switch  [q] Quit '
            elif st.tab == 2:
        ...
        def render_status(out, width, height):
            ...
            for i in range(vis):
                ...
                if item['type'] == 'header':
                    ...
                    label = item['label']
                    if idx == st.sel:
                        out.append('\033[7m   > ' + label + ' \033[0m')
                    else:
                        out.append('   \033[36m> ' + label + '\033[0m')
                else:
                    e = item['entry']
                    if item['type'] == 'staged':
        ...
        def render_cherry_pick(out, width, height):
            vis = height - 9
            sel_count = sum(1 for f in st.cherry_files if f['selected'])

            # Commit info
            out.append('\033[4;1H\033[K \033[1mPick files from:\033[0m \033[36m' + st.cherry_hash[:7] + '\033[0m ' + st.cherry_subject[:width-25])
            out.append('\033[5;1H\033[K \033[90mSelected: ' + str(sel_count) + '/' + str(len(st.cherry_files)) + ' files\033[0m')
            out.append('\033[6;1H\033[K\033[90m' + ('-' * width) + '\033[0m')

            if not st.cherry_files:
                out.append('\033[8;4H\033[90mNo files in this commit.\033[0m')
                for i in range(1, vis):
                    out.append('\033[' + str(8+i) + ';1H\033[K')
                return

            for i in range(vis):
                row = 7 + i
                out.append('\033[' + str(row) + ';1H\033[K')
                idx = st.cherry_scroll + i
                if idx >= len(st.cherry_files):
                    continue

                f = st.cherry_files[idx]
                status_icon = {'A': '+', 'M': '~', 'D': '-', 'R': '>'}.get(f['status'], '?')
                status_color = {'A': '32', 'M': '33', 'D': '31', 'R': '36'}.get(f['status'], '90')

                if idx == st.cherry_sel:
                    mark = '[x]' if f['selected'] else '[ ]'
                    out.append('\033[7m ' + mark + ' ' + status_icon + ' ' + f['path'][:width-10] + ' \033[0m')
                else:
                    if f['selected']:
                        check = '\033[32m[x]\033[0m'
                    else:
                        check = '\033[90m[ ]\033[0m'
                    out.append(' ' + check + ' \033[' + status_color + 'm' + status_icon + '\033[0m ' + f['path'][:width-10])
        ...
        def do_push():
            ...
            if rc == 0:
                st.msg = 'Pushed successfully'
                st.msg_color = '32'
            else:
                st.msg = 'Push failed: ' + (err or out)[:60]
                st.msg_color = '31'
            refresh_branch_info()
        ...
        def do_pull():
            ...
            st.msg_color = '36'
            render()
            rc, out, err = run_git('pull')
            if rc == 0:
                st.msg = 'Pulled successfully'
                st.msg_color = '32'
            else:
                st.msg = 'Pull failed: ' + (err or out)[:60]
                st.msg_color = '31'
            refresh_all()
        ...
        def do_fetch():
            ...
            st.msg_color = '36'
            render()
            rc, out, err = run_git('fetch', '--all')
            if rc == 0:
                st.msg = 'Fetched all remotes'
                st.msg_color = '32'
            else:
                st.msg = 'Fetch failed: ' + (err or out)[:60]
                st.msg_color = '31'
            refresh_branch_info()
            if st.tab == 2:
                st.branches = load_branches()

        def enter_cherry_pick(commit):
            """Enter cherry-pick file selection mode for a commit."""
            st.cherry_hash = commit['hash']
            st.cherry_subject = commit['subject']
            st.cherry_files = load_commit_files(commit['hash'])
            st.cherry_sel = 0
            st.cherry_scroll = 0
            st.mode = 'cherry_pick'
            st.msg = ''
        ...
        def apply_cherry_pick():
            """Apply selected files from the cherry-pick commit."""
            selected = [f for f in st.cherry_files if f['selected']]
            if not selected:
                st.msg = 'No files selected'
                st.msg_color = '33'
                return

            paths = [f['path'] for f in selected]
            rc, _, err = run_git('checkout', st.cherry_hash, '--', *paths)
            if rc == 0:
                st.msg = 'Applied ' + str(len(selected)) + ' file(s) from ' + st.cherry_hash[:7]
                st.msg_color = '32'
                st.mode = 'list'
                refresh_status()
                sys.stdout.write('\033[2J')
        ...
        def show_diff_vs_main():
            # Try main, then master
            for base in ('main', 'master'):
                rc, _, _ = run_git('rev-parse', '--verify', base)
                if rc == 0:
                    _, out, _ = run_git('diff', base + '...HEAD', '--color=never', '--stat')
                    _, patch, _ = run_git('diff', base + '...HEAD', '--color=never')
                    header = ['Diff: ' + st.branch_name + ' vs ' + base, '']
                    if out.strip():
                        header += out.splitlines() + ['', '--- full diff ---', '']
                    st.diff_lines = header + patch.splitlines()
                    st.prev_mode = 'list'
                    st.mode = 'diff'
                    st.diff_scroll = 0
                    sys.stdout.write('\033[2J')
                    return
            st.msg = 'No main or master branch found'
            st.msg_color = '33'
        ...
        def handle_input(c):
            ...
            # Cherry-pick file selection mode
            if st.mode == 'cherry_pick':
                if c == 'q' or c == '\x03':
                    return False
                if c == 'b':
                    st.mode = 'list'
                        ...
                        if c2 == '[':
                            ...
                            if c3 == 'A':  # Up
                                if st.cherry_sel > 0:
                                    st.cherry_sel -= 1
                                _, h = get_size()
                                if st.cherry_sel < st.cherry_scroll:
                                    st.cherry_scroll = st.cherry_sel
                            elif c3 == 'B':  # Down
                                if st.cherry_sel < len(st.cherry_files) - 1:
                                    st.cherry_sel += 1
                                _, h = get_size()
                                vis = max(1, h - 9)
                                if st.cherry_sel >= st.cherry_scroll + vis:
                                    st.cherry_scroll = st.cherry_sel - vis + 1
                    else:
                ...
                if c == 'j':
                    ...
                    # Toggle selection
                    if st.cherry_files and st.cherry_sel < len(st.cherry_files):
                        st.cherry_files[st.cherry_sel]['selected'] = not st.cherry_files[st.cherry_sel]['selected']
                elif c == 'a':
                    # Select all
                    for f in st.cherry_files:
                        f['selected'] = True
                    st.msg = 'Selected all files'
                    st.msg_color = '36'
                elif c == 'n':
                    # Select none
                    for f in st.cherry_files:
                        f['selected'] = False
                    st.msg = 'Deselected all files'
                    st.msg_color = '36'
                elif c == 'd':
                    # Show diff for selected file
                    if st.cherry_files and st.cherry_sel < len(st.cherry_files):
                        f = st.cherry_files[st.cherry_sel]
                        _, diff_out, _ = run_git('show', st.cherry_hash, '--', f['path'], '--format=')
                        st.diff_lines = diff_out.splitlines()
                        st.prev_mode = 'cherry_pick'
                        st.mode = 'diff'
                        st.diff_scroll = 0
                        st.msg = ''
                        sys.stdout.write('\033[2J')
            ...
            if st.mode in ('diff', 'commit_detail'):
                if c == 'q' or c == '\x03':
                    return False
                # 'd' in commit_detail → enter cherry-pick file picker
                if c == 'd' and st.mode == 'commit_detail' and st.commits and st.sel < len(st.commits):
                    enter_cherry_pick(st.commits[st.sel])
                    return True
                if c == 'b' or c == '\x1b':
                    if c == '\x1b':
                        if select.select([fd], [], [], 0.05)[0]:
                    ...
                    st.mode = st.prev_mode if st.prev_mode != 'diff' else 'list'
                    st.prev_mode = 'list'
                    st.diff_scroll = 0
                    sys.stdout.write('\033[2J')
                    return True
            ...
            if c == '\t':
                ...
                st.msg = 'Refreshed'
                st.msg_color = '36'
            elif c == 'p' and st.tab == 1:
                # Cherry-pick file selection from Log tab
