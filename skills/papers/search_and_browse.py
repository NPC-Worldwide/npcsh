...
def log(msg):
    sys.stderr.write(f"DEBUG [papers] {msg}\n")
    sys.stderr.flush()
...
def fetch_arxiv(query, limit):
    ...
    try:
        ...
        url = f"https://export.arxiv.org/api/query?search_query={search_query}&start=0&max_results={limit}&sortBy=relevance&sortOrder=descending"
    ...
    try:
        ...
        for attempt in range(5):
            ...
            try:
                ...
                if attempt < 4 and (response is None or response.status_code in (429, 503)):
                    delay = 10 * (2 ** attempt)
...
def fetch_wikipedia(query, limit):
    ...
    try:
        ...
        req = urllib.request.Request(f"{url}?{urllib.parse.urlencode(params)}", headers={"User-Agent": "npcsh/1.0"})
...
if text_only and query:
    all_papers = fetch_arxiv(query, limit) + fetch_openreview(query, limit)
    results = []
    ...
    TABS = [
        ...
        {'name': 'OpenReview', 'key': '2', 'fetch': fetch_openreview, 'color': '\033[35m'},
    ]
    ...
    try:
        ...
        while True:
            ...
            if mode == 'search':
                ...
                sys.stdout.write(f'  \033[2marXiv  ·  OpenReview\033[0m\n')
                ...
                footer = f' j/k:Nav  Enter:Detail  o:Open  p:PDF  d:DL  v:View  s:Sort  Tab:Next  1-2:Jump  /:Search  q:Quit {pos} '
