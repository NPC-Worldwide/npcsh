...
if not sys.stdin.isatty():
    ...
    def render():
        ...
        if st.approved_count or st.rejected_count:
            stats = f"  [+{st.approved_count} -{st.rejected_count}]"
        header = f" MEMORIES ({len(st.memories)}){stats} "
        out.append(f"\033[1;1H\033[K\033[7;1m{header.ljust(width)}\033[0m")

        # Tabs
        tab_str = ""
        for i, tab in enumerate(st.tabs):
            if i == st.tab:
                tab_str += f"\033[1;7m [{tab}] \033[0m"
            else:
                tab_str += f" \033[90m{tab}\033[0m "
        out.append(f"\033[2;1H\033[K {tab_str}")

        # Separator
        out.append(f"\033[3;1H\033[K\033[90m{'-' * width}\033[0m")
        ...
        out.append(f"\033[{height-1};1H\033[K")
        if st.msg:
            out.append(f" \033[{st.msg_color};1m{st.msg[:width-2]}\033[0m")
        ...
        out.append(f"\033[{height};1H\033[K\033[7m{foot.ljust(width)}\033[0m")
