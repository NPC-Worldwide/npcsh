...
if resource_type == 'npc':
    if not name:
        ...
        if os.path.exists(npc_path):
            ...
            jinxes_str = {{ jinxes | tojson }}.strip()
            if jinxes_str:
                jinxes_list = [j.strip() for j in jinxes_str.split(',')]
            else:
                jinxes_list = ['lib/core/python', 'lib/core/sh', 'lib/core/edit_file', 'lib/utils/chat', 'lib/utils/stop']

            npc_data = {'name': name, 'primary_directive': directive, 'jinxes': jinxes_list}
            if npc_model:
    ...
    if not name:
        ...
        jinx_dir = os.path.join(team_path, 'jinxes', sub_path)
