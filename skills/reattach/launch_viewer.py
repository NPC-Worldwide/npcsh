        ...
        while True:
            ...
            if c == 'q' or c == '\x03':
                ...
                if 'state' in dir() and state is not None:
                    ...
                    # Load past messages into state.messages so agents have context
                    loaded_msgs = []
                    try:
                        if hasattr(state, 'command_history') and state.command_history is not None:
                            db_msgs = state.command_history.get_conversations_by_id(cid)
                        else:
                            from npcpy.memory.command_history import CommandHistory
                            ch = CommandHistory()
                            db_msgs = ch.get_conversations_by_id(cid)
                        for m in (db_msgs or []):
                            role = m.get('role', '')
                            content = m.get('content', '')
                            if role in ('user', 'assistant', 'system', 'tool') and content:
                                loaded_msgs.append({"role": role, "content": content})
                    except Exception:
                        pass
                    state.messages = loaded_msgs
                    msg_count = len(loaded_msgs)
                    # Reset compression tracking since we loaded an existing conversation
                    state._last_compress_prompted_pct = 0
                    # Build replay of last 10 messages into output
                    replay = []
                    for m in loaded_msgs[-10:]:
                        r, c = m.get('role',''), m.get('content','')
                        if r == 'user':
                            replay.append(f"\033[32m> {c}\033[0m")
                        elif r == 'assistant':
                            replay.append(c)
                    replay.append(f"Reattached to: {cid} ({msg_count} messages loaded)")
                    context['output'] = '\n\n'.join(replay)
                    # Return messages in context so the router preserves them
                    context['messages'] = loaded_msgs
                else:
