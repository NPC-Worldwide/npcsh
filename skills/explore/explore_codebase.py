...
def llm(prompt, temperature=0.3):
    ...
    print(colored(f"    LLM: {elapsed:.1f}s", "white", attrs=["dark"]))
    return result
        ...
        with open(path, 'r', errors='replace') as f:
            ...
            for i, line in enumerate(f):
                if i >= max_lines_to_read:
                    lines.append(
                        f"\n... [truncated at {max_lines_to_read} lines, file continues] ..."
                    )
                    break
...
for root, dirs, files in os.walk(base_path):
    ...
    n_files = len(files)
    tree_lines.append(f"{indent}{dirname}/ ({n_files} files)")
...
for f in sorted(readable_files, key=lambda x: x['rel']):
    sz_str = f"{f['size'] / 1024:.1f}K" if f['size'] >= 1024 else f"{f['size']}B"
    file_listing_lines.append(f"  {f['rel']} ({sz_str})")
if len(file_listing_lines) > 120:
    file_listing = '\n'.join(file_listing_lines[:80])
    file_listing += f"\n  ... ({len(file_listing_lines) - 80} more files) ..."
else:
...
print(colored(f"\n  {len(readable_files)} readable files across {dir_count} directories", "green"))
print(colored(f"  Top types: {ext_summary[:80]}", "white", attrs=["dark"]))
...
print(colored(f"\n  Starting with {len(files_to_read)} files: {', '.join(os.path.basename(f) for f in files_to_read)}", "yellow"))
...
while iteration < max_iters and files_to_read:
    iteration += 1
    status(f"  Iteration {iteration}/{max_iters} - reading {len(files_to_read)} files...")
    ...
    for rel_path in files_to_read[:4]:  # Cap at 4 per round to keep prompts manageable
        ...
        print(colored(f"    Read: {rel}", "white", attrs=["dark"]))
    ...
    analysis_parts = [
        f"You are exploring a codebase at: {base_path}",
        f"Goal: {task_desc}" if task_desc else "",
        "",
        "FILES ALREADY READ:",
        files_read_list,
        findings_summary,
    ...
    print(colored(f"    Iteration {iteration} done. {len(files_read)} files read total. Next: {len(next_files)} files", "green"))
...
findings_text = '\n'.join(f"- {f}: {s}" for f, s in findings)
files_read_text = '\n'.join('  - ' + get_relative(f) for f in sorted(files_read))

task_section = ""
if task_desc:
    task_section = f'8. **Task Answer**: Directly answer: "{task_desc}"'

synth_parts = [
    f"You have explored a codebase at: {base_path}",
    f"The user specifically asked: {task_desc}" if task_desc else "",
    "",
    "STRUCTURE:",
    structure_report,
    "",
    f"FILES READ ({len(files_read)} total):",
    files_read_text,
    "",
    "FINDINGS:",
    findings_text,
...
print(colored(f"  {base_path} | {len(files_read)} files read across {iteration} iterations", "white", attrs=["dark"]))
print(colored(f"{sep}\n", "cyan"))
try:
