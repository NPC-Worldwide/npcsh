...
def render_screen():
    ...
    if state.inspecting and state.inspecting in user_vars:
        ...
        if isinstance(val, pd.DataFrame):
            ...
            header = f"     {''.join(f'{str(c)[:col_w]:<{col_w}}' for c in cols)}"
        ...
        if not state.output_lines:
            hints = [
                ...
                "Drop file paths or 'run file.py' to auto-load/execute", "",
                "\033[33mKeys:\033[0m Tab:NL  Ctrl+P:panels  Arrows:nav",
                "Ctrl+D:del  Ctrl+Q:quit",
            ]
    ...
    if state.panel == 0:  # Variables
        ...
        if state.plots:
            ...
            out.append(f"\033[6;{right_x}H\033[90mCall save_plot() to save\033[0m")
    ...
    # ===== FOOTER =====
    hints = "Tab:NL  Ctrl+P:panels  Arrows:nav  Ctrl+D:del  Ctrl+Q:quit"
...
def handle_input(c):
    if c == '\x11':  # Ctrl+Q - quit
    ...
    elif c == '\t':  # Tab - toggle natural language mode
        state.mode = "natural" if state.mode == "code" else "code"
        state.status = f"{state.mode} mode"
        state.selected_var = 0 # Reset selection when mode changes
        state.var_scroll = 0

    elif c == '\x10':  # Ctrl+P - cycle panels (new key for panels)
        ...
        if state.current_input.strip():
            ...
            if os.path.exists(os.path.expanduser(code.strip("'\""))):
            ...
            elif code.startswith("run ") and code.endswith(".py"):
                file_path = code[4:].strip()
                try:
                    with open(file_path, 'r') as f:
                        file_content = f.read()
                    state.output_lines.append(f">>> Running file: {file_path}")
                    output, error = execute_code(file_content)
                    if output:
                        state.output_lines.extend(output.split('\n'))
                    if error:
                        state.output_lines.extend(error.split('\n'))
                        state.status = "Error running file"
                    else:
                        state.status = f"Successfully ran {file_path}"
                except FileNotFoundError:
                    state.output_lines.append(f"Error: File not found: {file_path}")
                    state.status = "File not found"
                except Exception as e:
                    state.output_lines.append(f"Error reading file {file_path}: {e}")
                    state.status = "Error reading file"
                ...
                var_context = f"Variables: {', '.join(f'{k}({var_info(k,v)})' for k,v in get_user_vars().items())}"
