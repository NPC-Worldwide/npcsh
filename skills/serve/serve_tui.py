...
if not sys.stdin.isatty():
    ...
    ENDPOINT_REGISTRY = [
        ...
        # Jinxes
        {'category': 'Jinxes', 'method': 'GET', 'path': '/api/jinxes/available', 'desc': 'List available jinxes'},
        {'category': 'Jinxes', 'method': 'POST', 'path': '/api/jinx/execute', 'desc': 'Execute a jinx'},
        {'category': 'Jinxes', 'method': 'POST', 'path': '/api/jinx/test', 'desc': 'Test a jinx'},
        {'category': 'Jinxes', 'method': 'POST', 'path': '/api/jinxes/save', 'desc': 'Save a jinx'},
        {'category': 'Jinxes', 'method': 'GET', 'path': '/api/jinxes/global', 'desc': 'List global jinxes'},
        {'category': 'Jinxes', 'method': 'GET', 'path': '/api/jinxes/project', 'desc': 'List project jinxes'},
        # NPC Team
    ...
    def get_conflict_info(port):
        out, _, rc = run_cmd(f"lsof -i :{port} -P -n 2>/dev/null")
        if rc == 0 and out:
        ...
        out2, _, rc2 = run_cmd(f"ss -tlnp 'sport = :{port}' 2>/dev/null")
        if rc2 == 0 and out2:
    ...
    def kill_conflict(port):
        out, _, rc = run_cmd(f"lsof -t -i :{port} 2>/dev/null")
        if rc == 0 and out:
            ...
            for pid in pids:
                try:
                    ...
                    pass
            ui.status = f"Sent SIGTERM to port {port} processes"
            ui.conflict_info = ""
        else:
            ui.status = f"Could not find process on port {port}"
    ...
    def start_server():
        ...
        if is_port_in_use(ui.port):
            ...
            ui.status = f"Port {ui.port} in use{f' by {info}' if info else ''}"
            return
        ...
        try:
            ...
            ui.conflict_info = ""
            ui.status = f"Server started on port {ui.port}"
        except Exception as e:
            ui.status = f"Start failed: {str(e)[:60]}"
    ...
    def stop_server():
        ...
        try:
            ...
            ui.status = f"Stop failed: {str(e)[:40]}"
    ...
    def set_param_value(idx, val):
        ...
        if p == 'port':
            try:
                ui.port = int(val)
                ui.status = f"Port set to {val}"
            except:
                ui.status = "Invalid port number"
        elif p == 'host':
            ui.host = val
            ui.status = f"Host set to {val}"
        elif p == 'cors':
            ui.cors = val
            ui.status = f"CORS set to {val}"
        elif p == 'debug':
            ui.debug = val.lower() in ('on', 'true', '1', 'yes')
            ui.status = f"Debug {'on' if ui.debug else 'off'}"
    ...
    def render():
        ...
        if ui.tab == 0:
            ...
            if running:
                info = f"  \033[32mRunning\033[0m on \033[1m{ui.host}:{ui.port}\033[0m"
                ut = uptime_str()
                if ut:
                    info += f"  |  uptime: {ut}"
            else:
                ...
                if ui.conflict_info:
                    info += f"  |  \033[33mPort {ui.port} conflict: {ui.conflict_info}\033[0m"
            out.append(wline(4, info))
        elif ui.tab == 1:
            ct = len(ui.filtered_endpoints)
            total = len(ui.endpoints)
            if ui.search_query:
                info = f'  {ct} matching (of {total}) | search: "{ui.search_query}"'
            else:
                info = f"  {total} endpoints"
            out.append(wline(4, info))
        elif ui.tab == 2:
            info = f"  {len(ui.logs)} log entries"
            if server_running():
        ...
        if ui.confirm_action:
            out.append(wline(H - 1, f" \033[33m{ui.confirm_action[0]} (y/n)?\033[0m"))
        elif ui.search_mode:
    ...
    def render_server(out, W, start, body_h):
        ...
        for i, p in enumerate(ui.params):
            ...
            label = p.ljust(10)
            if i == ui.sel:
                lines.append((f"\033[7m > {label} : {val} \033[0m", True))
            else:
                lines.append((f"   \033[1m{label}\033[0m : {val}", False))

        # Extra info lines after params
        lines.append(("", False))
        running = server_running()
        if running:
            lines.append((f"   \033[1mURL\033[0m        : http://{ui.host}:{ui.port}", False))
            ut = uptime_str()
            if ut:
                lines.append((f"   \033[1mUptime\033[0m     : {ut}", False))
        else:
            if ui.conflict_info:
                lines.append((f"   \033[33mPort conflict\033[0m: {ui.conflict_info}", False))
        ...
        if ui.cors and ui.cors.strip():
            lines.append((f"   \033[1mCORS\033[0m       : {ui.cors}", False))
    ...
    def render_endpoints(out, W, start, body_h):
        ...
        for r in range(body_h):
            ...
            if idx == ui.sel:
                line = f" > {ep['method'].ljust(7)} {path_str}"
                remaining = W - len(line) - 3
                if remaining > 5:
                    line += f"  {desc_str[:remaining]}"
                out.append(wline(row, f"\033[7m{line[:W].ljust(W)}\033[0m"))
            else:
                cat_tag = ""
                if r == 0 or (r > 0 and vis[r-1]['category'] != ep['category']):
                    cat_tag = f"\033[90m[{ep['category']}]\033[0m "
                base = f"   {method_str} {path_str}"
                remaining = W - 14 - len(ep['path'])
                if remaining > 5 and not cat_tag:
                    base += f"  \033[90m{desc_str[:remaining]}\033[0m"
                elif cat_tag:
                    base = f"   {cat_tag}{method_str} {path_str}"
    ...
    def render_logs(out, W, start, body_h):
        ...
        for r in range(body_h - 1):
            ...
            if idx == ui.sel:
                line = f" > {entry['time'].ljust(10)} {entry['method'].ljust(8)} {entry['path'][:34].ljust(34)} {entry['status']}"
                out.append(wline(row, f"\033[7m{line[:W].ljust(W)}\033[0m"))
            else:
                out.append(wline(row, f"   {t} {m} {p} {s}"))
    ...
    def show_endpoint_detail():
        ...
        ui.detail_lines = [
            ...
            "",
            f"\033[1mCategory:\033[0m  {ep['category']}",
            f"\033[1mMethod:\033[0m    {ep['method']}",
            f"\033[1mPath:\033[0m      {ep['path']}",
            f"\033[1mDesc:\033[0m      {ep['desc']}",
            "",
        ]
        if server_running():
            url = f"http://{ui.host}:{ui.port}{ep['path']}"
            ui.detail_lines.append(f"\033[1mFull URL:\033[0m  {url}")
        ui.detail = True
        ui.detail_scroll = 0
    ...
    def handle_server_key(c):
        if c == 's':
            ...
            def on_port(val):
                try:
                    ...
                    if is_port_in_use(ui.port):
                        info = get_conflict_info(ui.port)
                        ui.conflict_info = info
                        ui.status += f" (in use{f' by {info}' if info else ''})"
                    else:
                        ui.conflict_info = ""
                except:
                    ui.status = "Invalid port number"
            ...
            if ui.conflict_info or is_port_in_use(ui.port):
                ...
                ui.confirm_action = (f"Kill process on port {ui.port} ({ui.conflict_info})",
    ...
    def handle_search(c):
        if c == '\x1b':
            ...
            filter_endpoints()
            if ui.search_query:
                ui.status = f'Filter: "{ui.search_query}" ({len(ui.filtered_endpoints)} results)'
            else:
                ui.status = "Search cleared"
        elif c in ('\x7f', '\x08'):
