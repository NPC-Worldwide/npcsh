    ...
    if not tables_ok:
        ...
        def load_fact_detail(idx):
            ...
            with engine.connect() as conn:
                ...
                found = False
                for row in r:
                    lines.append((f"  \033[36m{row.target or ''}\033[0m", ""))
                    found = True
                if not found:
        ...
        def load_concept_detail(idx):
            ...
            lines = []
            lines.append((f"\033[1mConcept:\033[0m \033[36m{concept['name']}\033[0m", ""))
            ...
            with engine.connect() as conn:
                ...
                found2 = False
                for row in r:
                    lines.append((f"  \033[36m{row[0] or ''}\033[0m", ""))
                    found2 = True
                if not found2:
                    lines.append(("  \033[90m(none)\033[0m", ""))
            ui.detail_lines = lines
            ...
            if item['kind'] == 'fact':
                ...
                for ci, c in enumerate(ui.concepts):
                    if c['name'] == item['text']:
                        ...
                        return
                ui.detail_lines = [
                    (f"\033[1mConcept:\033[0m \033[36m{item['text']}\033[0m", ""),
                    (f"\033[1mOrigin:\033[0m {item['origin']}", ""),
                    (f"\033[1mGeneration:\033[0m {item['generation']}", ""),
                ]
