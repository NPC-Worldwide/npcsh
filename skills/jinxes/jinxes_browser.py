...
if not sys.stdin.isatty():
    context['output'] = "Jinxes browser requires an interactive terminal."
    ...
    # ── data loading ─────────────────────────────────────
    def load_jinxes():
        """Load jinxes from team directory and global ~/.npcsh directory."""
        items = []
        seen_paths = set()
        ...
        # Team jinxes
        team_dir = Path(state.team.team_path) / 'jinxes'
        scan_dir(team_dir, 'team')

        # Global jinxes
        global_dir = Path.home() / '.npcsh' / 'npc_team' / 'jinxes'
    ...
    def render():
        ...
        total = len(ui.all_jinxes)
        if ui.search_query:
            count_text = f"  {count} matching (of {total}) | search: \"{ui.search_query}\""
        elif ui.tab == 0:
            count_text = f"  {total} jinxes loaded"
        else:
            count_text = f"  {count} jinxes ({ui.tabs[ui.tab].lower()})"
    ...
    def render_list(out, W, start, body_h):
        ...
        if not ui.filtered:
            out.append(wline(start, "  \033[90mNo jinxes match the current filter.\033[0m"))
    ...
    context['output'] = "Jinxes browser closed."
