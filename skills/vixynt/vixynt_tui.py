...
img_dir = os.path.expanduser("~/.npcsh/npc_team/images/")
...
def generate_images(silent=False):
    ui.mode = 'generating'
    ...
    try:
        ...
        for i, image in enumerate(images_list):
            ...
            if ui.output_name and ui.output_name.strip():
                ...
                if not ext:
                    ext = ".png"
                out_file = f"{base}{'_' + str(i) if len(images_list) > 1 else ''}{ext}"
            else:
                os.makedirs(img_dir, exist_ok=True)
                out_file = os.path.join(img_dir, f"vixynt_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{i}.png")
            image.save(out_file)
            saved.append(out_file)
            ui.gallery.insert(0, {"path": out_file, "prompt": ui.prompt, "timestamp": os.path.basename(out_file)})

        ui.status = f"Generated {len(saved)} image(s): {', '.join(os.path.basename(f) for f in saved)}"
    except Exception as e:
        ui.status = f"Error: {str(e)[:60]}"
...
def render_screen():
    ...
    if ui.mode in ('params', 'editing', 'generating'):
        # Parameter panel
        out.append(f"\033[3;1H\033[36;1m Parameters \033[90m{'-' * (width - 13)}\033[0m")
        ...
        for i, p in enumerate(ui.params):
            ...
            if ui.mode == 'editing' and i == ui.sel:
                line = f"  {label.ljust(14)}\033[7m {ui.edit_buf} \033[0m"
            else:
                line = f"  {label.ljust(14)}{val[:width - 18]}"
        ...
        out.append(f"\033[{gallery_row};1H\033[33;1m Gallery ({len(ui.gallery)}) \033[90m{'-' * (width - 20)}\033[0m")
        ...
        out.append(f"\033[3;1H\033[33;1m Gallery ({len(ui.gallery)}) \033[90m{'-' * (width - 20)}\033[0m")
        ...
        for i in range(gallery_h):
            ...
            if idx == ui.gallery_sel:
                out.append(f"\033[7m> {fname[:width-4]}\033[0m")
            else:
                out.append(f"  {fname[:width-4]}")

    # Status + footer
    out.append(f"\033[{height-2};1H\033[K\033[90m{'-' * width}\033[0m")
    out.append(f"\033[{height-1};1H\033[K {ui.status[:width-2]}")
    ...
    out.append(f"\033[{height};1H\033[K\033[7m{footer.ljust(width)}\033[0m")
...
def handle_gallery(c, fd):
    ...
    if c == 'q':
        ...
        try:
            ...
            ui.status = f"Opened: {os.path.basename(path)}"
        except:
...
if ui.prompt and context.get('prompt'):
    generate_images(silent=True)
    context['output'] = ui.status
    context['messages'] = context.get('messages', [])

# ========== Main TUI Loop ==========
elif sys.stdin.isatty() and (tty is not None and termios is not None):
    fd = sys.stdin.fileno()
    ...
    context['output'] = "No prompt provided. Use: npc vixynt \"your prompt here\""
    context['messages'] = context.get('messages', [])
